        ...
        <a-table :columns="columns" :data-source="tableData" :pagination="false">
            <template #bodyCell="{ column, record }">
                ...
                <template v-else-if="column.key === 'totalAmount'">
                    <div>
                        <div class="total-yj">原价：{{ (record.unitPrice * record.quantity).toFixed(2) }}</div>
                        <div>折后价：<span class="total-amount"> <span>￥</span>{{ record.totalAmount }}</span></div>
                    </div>
                </template>
...
import { list, del, save, changeStatus } from '~/api/retailer/order.js'
