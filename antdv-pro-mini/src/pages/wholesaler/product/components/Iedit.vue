<template>
  <a-modal v-model:open="visible" :title="modelRef.id ? '编辑' : '新增'" destroyOnClose :confirm-loading="confirmLoading" @ok="handleOk"
...
import { save } from '~/api/growers/inventoryP.js'
const formRef = ref();
  ...
  try {
    ...
    if (rules.length > 0) {
      ...
      for (let i = 1; i < sortedRules.length; i++) {
        if (sortedRules[i].amount <= sortedRules[i - 1].amount) {
          message.error(`第${i + 1}层的数量必须大于第${i}层的数量`);
          return;
        }
      }
      ...
      // 检查每一层的折扣是否基于上一层递减
      for (let i = 1; i < sortedRules.length; i++) {
        if (sortedRules[i].discount >= sortedRules[i - 1].discount) {
          message.error(`第${i + 1}层的折扣必须小于第${i}层的折扣`);
          return;
        }
      }
    }
