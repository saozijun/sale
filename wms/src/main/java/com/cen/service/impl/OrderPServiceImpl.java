...
import com.cen.mapper.OrderPMapper;
 ...
 */
@Service
public class OrderPServiceImpl extends ServiceImpl<OrderPMapper, OrderP> implements IOrderPService {
    @Resource
    ...
    public Page<OrderDetailDTO> findOrderDetailPage(Integer pageNum, Integer pageSize, Long userId, String status) {
        ...
        // 创建分页对象
        Page<OrderP> page = new Page<>(pageNum, pageSize);

        // 创建查询条件
        QueryWrapper<OrderP> queryWrapper = new QueryWrapper<>();
        ...
        // 查询订单数据
        Page<OrderP> orderPage = page(page, queryWrapper);
        List<OrderP> orderList = orderPage.getRecords();
        ...
        List<Long> productIds = orderList.stream()
                .map(OrderP::getProductId)
                .filter(id -> id != null)
                .collect(Collectors.toList());

        List<Long> buyerIds = orderList.stream()
                .map(OrderP::getBuyerId)
                .filter(id -> id != null)
                .collect(Collectors.toList());

        List<Long> sellerIds = orderList.stream()
                .map(OrderP::getSellerId)
                .filter(id -> id != null)
                .collect(Collectors.toList());

        // 批量查询相关数据
        Map<Long, ProductP> productMap;
        if (!productIds.isEmpty()) {
            List<ProductP> products = productPService.listByIds(productIds);
            productMap = products.stream()
                    .collect(Collectors.toMap(ProductP::getId, Function.identity()));
        } else {
        ...
        List<OrderDetailDTO> records = orderList.stream().map(order -> {
            ...
            // 设置商品信息
            ProductP productP = productMap.get(order.getProductId());
            if (productP != null) {
                dto.setProductName(productP.getName());
                dto.setProductImage(productP.getCoverImage());
            }
